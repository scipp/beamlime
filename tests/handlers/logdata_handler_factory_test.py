...
from beamlime.core.message import Message, MessageKey, StreamKind
...
def test_make_handler_with_valid_source(default_config, attribute_registry):
    ...
    key = MessageKey(kind=StreamKind.LOG, source_name="temperature_sensor")
...
def test_make_handler_with_missing_source(
    ...
    key = MessageKey(kind=StreamKind.LOG, source_name="nonexistent_sensor")
    handler = factory.make_handler(key)
...
def test_make_handler_with_invalid_attributes(
    ...
    key = MessageKey(kind=StreamKind.LOG, source_name="invalid_sensor")
    handler = factory.make_handler(key)
...
def test_logdata_handler_preserves_source_name(default_config, attribute_registry):
    ...
    key = MessageKey(kind=StreamKind.LOG, source_name=source_name)
    handler = factory.make_handler(key)
