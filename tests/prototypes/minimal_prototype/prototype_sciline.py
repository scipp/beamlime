...
from .random_data_providers import NumPixels, RandomEvents
from .workflow_sciline import (
    BinnedData,
    ChunkSize,
    Events,
    Histogrammed,
...
class DataStreamSimulator(BaseApp):
    raw_data_pipe: List[RandomEvents]
...
class DataReductionMixin(BaseApp, ABC, Generic[InputType, OutputType]):
    ...
    async def run(self) -> None:
        ...
        def data_provider():
            return data_sink.get()

        try:
            self.workflow._set_provider(self.input_type, data_provider)
        except ValueError:
            self.workflow._providers.pop(self.input_type)
            self.workflow._set_provider(self.input_type, data_provider)

        workflow = self.workflow.build(self.output_type)
        while (
            ...
            data_sink.put(data)
            self.output_pipe.append(workflow.get(self.output_type))
            await asyncio.sleep(0)
...
class Prototype(BaseApp):
    ...
    data_reduction: DataReduction[MergedData, Histogrammed]
...
def create_prototype(event_rate, num_pixels, chunk_size, frame_rate, num_frames):
    ...
    from .random_data_providers import (
        EventRate,
        FrameRate,
        NumFrames,
        RandomEvents,
    ...
    from .workflow_sciline import HistogramBinSize
    from .workflow_sciline import NumPixels as WorkflowNumPixels
    from .workflow_sciline import workflow_providers

    prototype_providers = ProviderGroup(Prototype)
    prototype_providers[DataMerge[Events, MergedData]] = DataMerge
    prototype_providers[DataReduction[MergedData, Histogrammed]] = DataReduction
    prototype_providers[DataStreamListener] = DataStreamSimulator

    for pipe_type in (RandomEvents, BinnedData, MergedData, ReducedData, Histogrammed):
    ...
    config_providers[WorkflowNumPixels] = lambda: num_pixels
