...
from typing import Any, Callable, Dict, Literal, NewType, Type, Union

DependencyType = NewType("DependencyType", object)
DependencyValue = NewType("DependencyValue", object)
...
ProductType = Union[object, Type[UnknownType]]


@dataclass
class DependencySpec:
    dependency_type: DependencyType
    default_value: DependencyValue
...
def extract_underlying_type(product_type: Any) -> Any:
...
class ProductSpec:
    def __new__(cls, product_type: Any) -> ProductSpec:
        if isinstance(product_type, ProductSpec):
            return product_type
        else:
            return super().__new__(ProductSpec)

    def __init__(self, product_type: Any) -> None:
